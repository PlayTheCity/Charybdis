...
function ThreadController( filename, autostart  )
{
	...
	this.registerFunction = function( funcname, paramscount )
	{
		if ( this.Initialized == false )
			return;
		alert('register function was called');
	...
	this.startThread = function()
	{
		...
		this.WebWorker.addEventListener('message',this.msgHandler, false);
		...
		this.WebWorker.postMessage(
		{
			'type' : 'command',
			'ex' : 'start',
			'regfunc' : this.registerFunction,
			'refunc' : this.removeFunction
		}
		);
	}
